<template>
  <div>
    <div class="filter-container">
      ...
      <el-button v-waves class="filter-item" type="info" @click="handleDeleteRole">删除</el-button>
      <el-input
        v-model="searchValue"
        clearable
        placeholder="搜索角色名称"
        ...
        @clear="initList"
      />
      <el-button v-waves class="filter-item" type="primary" @click="initList">查询</el-button>
    </div>
...
import { getRoleList, delRole } from '@/apis/system'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      const data = { pageNum, pageSize, name: searchValue }
      getRoleList(data)
        .then(res => {
    ...
    handleDeleteRole() {
      ...
      this.$confirm('确认删除？', {
        ...
        delRole({ ids: this.selection.map(item => item.id) })
          .then(res => {
    ...
    viewDetails(id, target) {
      this.$router.push({ path: '/system/role-' + target, query: { id, target }})
    }
  }
}
</script>
