<template>
  <div>
    ...
    <el-table :data="tableData" border stripe>
      ...
      <el-table-column show-overflow-tooltip align="center" prop="coupon" label="使用优惠券">
        <!-- <template v-slot="{row}">
          <el-tag v-for="(item, index) in row.coupon" :key="index">{{ item }}</el-tag>
        </template> -->
      </el-table-column>
    </el-table>
...
import { getSaleOrderDetails } from '@/apis/order'
...
export default {
  ...
  computed: {
    informationList() {
      ...
      const list = [
        { label: '订单号', value: data.sn, long: false },
        ...
        { label: '实际金额', value: data.actualPay, long: false },
        { label: '运费', value: data.postage, long: false },
        { label: '优惠券金额', value: data.discount, long: false },
        { label: '米宝', value: data.ecoin, long: false },
  ...
  methods: {
    getList() {
      getSaleOrderDetails({ sn: this.sn })
        .then(res => {
