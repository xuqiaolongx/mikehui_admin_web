...
import { ByAdvertisingSave, ByAdvertisingEdit, ByAdvertisingUpdate } from '@/apis/advertisement'
...
export default {
  ...
  methods: {
    submit() {
      ...
      if (!picture) return this.$message.error('请上传广告图片')
      if (!order) return this.$message.error('请输入轮播图的排序')
      if (!banner) return this.$message.error('请选择类型')
      if (!this.$route.query.id) {
        ByAdvertisingSave({ picture, order, banner, adName, type, code: id })
          .then(res => {
            this.$message.success('添加成功')
            this.$router.back(-1)
          })
      } else {
        ByAdvertisingUpdate({ picture, order, banner, adName, type, code: id, id: this.$route.query.id })
          .then(res => {
