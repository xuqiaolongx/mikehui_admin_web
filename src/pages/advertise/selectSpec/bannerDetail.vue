...
import { PickedSpecialitySave, PickedSpecialityEdit, PickedSpecialityUpdate } from '@/apis/advertisement'
  ...
  methods: {
    submit() {
      ...
      if (!this.$route.query.id) {
        PickedSpecialitySave({ picture, order, banner, adName, type, code: id })
          .then(res => {
            this.$message.success('添加成功')
            this.$router.back(-1)
          })
      } else {
        PickedSpecialityUpdate({ picture, order, banner, adName, type, code: id, id: this.$route.query.id })
          .then(res => {
    ...
    getAdvertiseInfo() {
      ...
      if (!id) return
      PickedSpecialityEdit({ id }).then(res => {
