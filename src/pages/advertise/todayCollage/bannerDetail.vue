...
import { TodayGroupSave, TodayGroupEdit, TodayGroupUpdate } from '@/apis/advertisement'
  ...
  methods: {
    submit() {
      ...
      if (!this.$route.query.id) {
        TodayGroupSave({ picture, order, banner, adName, type, code: id })
          .then(res => {
            this.$message.success('添加成功')
            this.$router.back(-1)
          })
      } else {
        TodayGroupUpdate({ picture, order, banner, adName, type, code: id, id: this.$route.query.id })
          .then(res => {
